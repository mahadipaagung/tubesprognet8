...
@section('content')
    <div class="nk-fmg-body-head d-none d-lg-flex">
        ...
        <div class="nk-fmg-actions">
            <div class="btn-group">
                <a href="{{ route('struktural.create', ['id' => $id]) }}" class="btn btn-sm btn-primary"
                    onclick="buttondisable(this)"><em class="icon fas fa-plus"></em> <span>Add Data</span></a>
            </div>
        </div>
    </div>
    <div class="nk-fmg-body-head d-none d-lg-flex">
        <div class="nk-fmg-search">
            <h4 class="card-title text-primary"><i class='' data-toggle='tooltip' data-placement='bottom'
                    title='Data {{ $nama }} '></i> {{ strtoupper('Nama ' . $nama) }}</h4>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#{{ $table_id }}').DataTable({
                ...
                ajax: {
                    url: '{{ url('pegawai/struktural/list') }}/' + {{ $id }},
                    type: "POST",
            ...
            $('.dataTables_filter').html(
                '<div><div class="input-group"><div class="input-group-prepend"><span class="input-group-text" id="basic-addon1"><em class="ti ti-search"></em></span></div><input type="search" class="form-control form-control-sm" placeholder="Type in to Search" aria-controls="tbtariflayanan"></div></div>'
            );
        });
